...
(define (codigo-secreto)
  ...
  (cond
    [(equal? escolha "VOLTAR") "recebe função de loop "]
    [(equal? escolha "DICA") (dica)]
    [else (codigo-secreto) ]))
...
(define escolha (string-upcase (read-line)))
(cond
    [(equal? escolha "VOLTAR") (codigo-secreto)]
    [else (dica)]))
        
    
(define (Colocar-codigo)
  (pular-linhas)
  ...
  (cond  [(equal? resposta "CODIGOSECRETO") (display "Resposta Correta!")]
         [(equal? resposta "CODIGO SECRETO") (display "Resposta Correta!")]
         [else
          "Resposta Errada"]))
...
(define (trivia)
  ...
  (if (> (soma (list(correto resposta1 "A") (correto resposta2 "D") (correto resposta3 "A") (correto resposta4 "C") (correto resposta5 "B"))) 2 )
      (display "Você passou no teste Parabens!")
      (display "Você é a vergonha da profissão")))
...
(define escolha (string-upcase (read-line)))
(cond
    [(equal? escolha "RESP") (Colocar-codigo2)]
    [(equal? escolha "VOLTAR") "recebe função de loop "]
    [(equal? escolha "DICA") (display "Dica: ANA + GRAMA")(newline)(anagrama)]
    [else (anagrama)]))

(define (Colocar-codigo2)
  (pular-linhas)
  (display "-----Insira a Resposta.------")(newline)(newline)
  (define resposta (string-upcase (read-line)))
  (cond  [(equal? resposta "RACKET") (display "Resposta Correta!")]
         [else
          "Resposta Errada" (anagrama) ]))
